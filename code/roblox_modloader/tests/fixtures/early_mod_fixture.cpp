...
namespace
{
	...
	class FixtureMod final : public ModBase
	{
		void on_load() override
		{
			++counts[2];
			if (mode == 6)
				throw std::runtime_error("on_load fixture");
	...
	const RmlClassRegistrationV1 registration{1, sizeof(RmlClassRegistrationV1), "FixtureClass", "Instance", &descriptor, &descriptor, nullptr, 0, 0};
}

extern "C" RML_MOD_ABI_EXPORT int fixture_count(int index) noexcept
{
	return counts[index];
}
...
#ifndef OMIT_START
extern "C" RML_MOD_ABI_EXPORT ModBase* start_mod()
{
	++counts[1];
	return new FixtureMod;
}
#endif
#ifndef OMIT_UNINSTALL
extern "C" RML_MOD_ABI_EXPORT void uninstall_mod(const ModBase* mod)
{
	++counts[4];
	delete mod;
}
...
extern "C" RML_MOD_ABI_EXPORT int rml_global_init(const RmlGlobalInitContext* context) noexcept
{
	...
	try
	{
		...
		if (mode == 3)
			RaiseException(EXCEPTION_ACCESS_VIOLATION, 0, 0, nullptr);
	#endif
		if (!context || context->abi_version != RML_GLOBAL_INIT_ABI_VERSION || !context->mod_root_utf8 || !context->studio_build_utf8
		    || !context->log)
			return 18;
		...
		++counts[6];
		if (mode == 10)
			return 0; // Deliberately leave an outstanding reservation.
		if (mode == 4)
			throw std::runtime_error("reserved batch");
	#if defined(_WIN32)
		if (mode == 5)
			RaiseException(EXCEPTION_ACCESS_VIOLATION, 0, 0, nullptr);
	#endif
		...
		api->commit_batch(batch);
		if (mode == 11)
			return 22; // Publication cannot be rolled back or unloaded.
		return 0;
	}
	catch (...)
	{
		++counts[5];
		if (batch)
			context->descriptors->abort_batch(batch);
		return 21;
	}
}
#endif
