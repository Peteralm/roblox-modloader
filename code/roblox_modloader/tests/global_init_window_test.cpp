...
namespace rml::platform::windows
{
	namespace
	{
		constexpr std::array<std::uint8_t, 30> kSignature{0x48, 0x89, 0x5C, 0x24, 0x20, 0x55, 0x56, 0x57, 0x41, 0x54, 0x41, 0x55, 0x41, 0x56, 0x41, 0x57, 0x48, 0x8D, 0x6C, 0x24, 0xD9, 0x48, 0x81, 0xEC, 0xA0, 0x00, 0x00, 0x00, 0xC6, 0x05};

		struct FakeImage
		{
			std::vector<std::byte> bytes = std::vector<std::byte>(0x2000);
			EmbeddedBootstrapProfile profile{0x12345678,
			    0x2000,
			    "fixture",
			    0x500,
			    0x1800,
