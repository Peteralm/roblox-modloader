...
namespace rml
{
	constexpr auto Pointers::get_roblox_batch()
	{
	    ...
	    {"GET_STRING_ATOM",
		    ...
		    // Fork: class registration's nine entry points are resolved through RTTI by
		    // reflection_anchors.cpp, not by the byte signatures upstream keeps here.
		    {"LUA_GETTOP",
		    ...
		    {"LUA_SETTOP",
		        "48 89 5C 24 ? 57 48 83 EC 20 4C 8B 41 ? 48 8B D9 48 63 FA 85 D2",
		        [](const memory::handle ptr) {
		    ...
		    {"LUA_PUSHNIL",
		        "40 53 48 83 EC 20 48 8B D9 48 8B 49 ? 48 8B 43",
		        [](const memory::handle ptr) {
			        g_pointers->m_roblox_pointers.lua_pushnil = ptr.as<functions::lua_pushnil>();
		        }
		    },
		    {"LUA_PUSHNUMBER",
		        "40 53 48 83 EC 30 48 8B D9 0F 29 74 24 ? 48 8B 49 ? 0F 28 F1 48 8B 43",
		        [](const memory::handle ptr) {
		    ...
		    {"LUA_PUSHCCLOSUREK",
		        "48 89 5C 24 ? 56 57 41 56 48 83 EC 20 4C 8B 51",
		        [](const memory::handle ptr) {
		    ...
		    {"LUAM_VISITGCO",
		        "40 ? 41 54 41 57 48 83 EC 30 48 8B 41 ? 4D 8B F8",
		        [](const memory::handle ptr) {
		    ...
		    {"LUAL_CHECKANY",
		        "48 89 5C 24 ? 57 48 83 EC 20 8B DA 48 8B F9 E8 ? ? ? ? 83 F8 FF 74 ? 48 8B 5C 24 ? 48 83 C4 20 5F C3 44 8B C3",
		        [](const memory::handle ptr) {
